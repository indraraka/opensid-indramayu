...
// Gunakan direktori sesi yang dijamin dapat ditulis. Pada sebagian server
// (mis. Apache/mod_fcgid di Windows) sys_get_temp_dir() menunjuk ke
// C:\Windows\Temp yang tidak writable, sehingga sesi gagal disimpan dan
// login admin tidak dapat dipertahankan. Jika temp dir tidak writable,
// jatuh ke folder storage aplikasi yang pasti writable.
$config['sess_save_path'] = is_writable(sys_get_temp_dir())
    ? sys_get_temp_dir()
    : FCPATH . STORAGEPATH . 'framework' . DIRECTORY_SEPARATOR . 'sessions';
if (! is_dir($config['sess_save_path'])) {
    @mkdir($config['sess_save_path'], 0775, true);
}
