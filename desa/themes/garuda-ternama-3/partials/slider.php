...
<section class="sliderx w-full relative group transition-all duration-300 overflow-hidden">
  <div class="owl-carousel rounded-lg z-10 relative w-full">
    <?php foreach($slider_gambar['gambar'] as $data) : ?>
    <?php
      // Gunakan gambar resolusi penuh (asli) bila ada, fallback ke versi sedang
      $img = $slider_gambar['lokasi'] . $data['gambar'];
      if (! is_file($img)) {
