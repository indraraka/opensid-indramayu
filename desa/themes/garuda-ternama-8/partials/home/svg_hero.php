...
<?php /* Garuda Ternama 8 — Motif TANI MAKMUR (padi, sawah, mangga, matahari panen) */ ?>
<section class="grd-container" style="margin-top:1.5rem;">
  <div class="grd-hero-motif grd-hero-motif--tani">
    <svg class="grd-hero-motif__art" viewBox="0 0 1440 320" preserveAspectRatio="xMidYMax slice" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ilustrasi sawah, padi, dan panen">
      <defs>
        <linearGradient id="gtp8-sky" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stop-color="#0D9488"/><stop offset="1" stop-color="#0B5F58"/>
        </linearGradient>
    ...
    <div class="grd-hero-motif__inner">
      <span class="grd-hero-motif__eyebrow"><i class="fas fa-tractor"></i> <?= theme_config('hero_eyebrow', 'Tani Makmur') ?></span>
      <h2 class="grd-hero-motif__title"><?= theme_config('hero_title', '') ?: ('Selamat Datang di ' . NAMA_DESA) ?></h2>
      <p class="grd-hero-motif__sub"><?= theme_config('hero_sub', 'Lumbung pangan dan kebun mangga — kemakmuran tumbuh dari tanah desa.') ?></p>
    </div>
  </div>
</section>
<style>
  ...
  .grd-hero-motif__eyebrow{display:inline-flex;align-items:center;gap:.4rem;background:rgba(101,163,13,.3);backdrop-filter:blur(3px);color:#fff;font-weight:700;font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;padding:.3rem .85rem;border-radius:9999px;}
  .grd-hero-motif__title{font-family:var(--grd-font-display,sans-serif);color:#fff;font-weight:800;font-size:clamp(1.4rem,1rem+2vw,2.4rem);margin:.65rem 0 .35rem;text-shadow:0 2px 14px rgba(120,53,15,.5);line-height:1.1;}
  .grd-hero-motif__sub{color:#fff;max-width:31rem;font-size:.95rem;text-shadow:0 1px 8px rgba(120,53,15,.55);margin:0;}
