<div class="container">
    @includeWhen(!empty($transparansi ?? null), 'theme::partials.apbdesa', $transparansi ?? [])
</div>
...
@include('theme::commons.chat_wa')
@include('theme::commons.footer_info')

<footer class="w-full pt-5 footer">
    <div class="bg-zinc-700 text-white py-5 px-5 lg:px-8 text-sm flex flex-col gap-3 lg:flex-row justify-between items-center text-center lg:text-left">
        <span class="space-y-2">
            <p>Hak cipta situs &copy; {{ date('Y') }} - {{ $nama_desa }}</p>
            <p>
                <a href="https://opensid.or.id" class="underline decoration-pink-500 underline-offset-1 decoration-2" target="_blank" rel="noopener">Garuda {{ $themeVersion }}</a> -
